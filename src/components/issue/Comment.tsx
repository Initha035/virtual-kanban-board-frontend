...
import type { AuthUser, Comment as CommentType } from "../../api/apiTypes";
...
function Comment(props: Props) {
  const { username, descr, profileUrl, createdAt, u, userId, id, projectId } =
    props;
      ...
      <li className="mb-6 flex items-start gap-3">
        ...
        <div className="grow">
          ...
          {isMine && (
            <button
              onClick={() => setOn(true)}
              className="text-sm tracking-wide text-red-700 hover:underline"
            >
              Delete
            </button>
          )}
